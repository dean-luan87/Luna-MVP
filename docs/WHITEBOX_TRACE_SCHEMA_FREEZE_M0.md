...
所有白盒轨迹与推理可视化的**统一入口**为 **Luna Reasoning Console**。  
...
- 结构树（Reasoning Structure Tree）是白盒之上的总组织结构，用于统一组织分支推理、排除路径、反馈驱动路径与收敛结果路径。
- 后续任何新功能，只要产生新的推理分支、排除路径、用户反馈驱动路径或结果收敛路径，都应逐步接入结构树；不得长期只存在于模块内部而不进入总结构树。
...
- **已接入（正式样板）**：
  ...
  - Confirmation 的白盒轨迹 `confirmation_whitebox_trace`（Confirmation Whitebox Trace M0，含用户可见解释层；单测 + smoke/JSONL 已闭环）。
