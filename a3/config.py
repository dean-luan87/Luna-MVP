...
@dataclass
class A3ModulationV1:
    """Phase4-MVP：alpha 调制器，默认关闭。"""
    enabled: bool = False
    lam: float = 0.10
    alpha_min: float = 0.55
    alpha_max: float = 0.80
    risk_density_alpha: float = 0.2
...
class A3Config:
    ...
    # B2 view_conf gate：gate = floor + (1-floor)*view_conf^k；k=1,floor=0.5 等价于旧逻辑 0.5+0.5*view_conf
    view_conf_gate_floor: float = 0.5
    view_conf_gate_k: float = 1.0
    modulation: A3ModulationV1 = field(default_factory=A3ModulationV1)
...
def from_flat_dict(flat: Dict[str, Any]) -> A3Config:
    ...
    modulation = sub(A3ModulationV1, nested.get("modulation", {}))
    top_only = {"enabled", "roi_count_cap", "branch_count_cap", "risk_scale_factor", "view_conf_gate_floor", "view_conf_gate_k"}
