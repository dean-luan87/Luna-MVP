# tests/test_b2_v041_gate_behavior_standalone.py
"""
B2 v0.4.1 行为回归测试脚本（独立版本，避免导入问题）
...
b2_path = os.path.join(os.path.dirname(__file__), '..', 'vision_pipeline', 'b2', 'v03')
...
try:
    ...
    print(f"❌ 导入错误: {e}")
    print("\n请确保在项目根目录运行此脚本：")
    print("  cd /Users/luanlei/Desktop/Luna-2")
    print("  python3 tests/test_b2_v041_gate_behavior_standalone.py")
    sys.exit(1)
...
def run_case(
    ...
    try:
        ...
        # 初始化 B2（禁用 trace 和 logger 以避免文件操作）
        b2 = B2v03(
        ...
        main_factor_name = main_factor.value if hasattr(main_factor, "value") else str(main_factor) if main_factor else None
        intervention_level = summary.get("intervention_level", "UNKNOWN")
        
        print(f"\n📊 B Output:")
        print(f"  Impact: {impact_name}")
        print(f"  Decision Level: {decision_level}")
        print(f"  Main Factor: {main_factor_name}")
