...
# -*- coding: utf-8 -*-
"""
D0.1: Headless A3 适配器。只读 obs dict + virtual_ts，只写输出 dict。
不碰 runtime；由 tools/run_a3_headless_replay 等调用。
"""
from __future__ import annotations

from typing import Any, Dict

from a3.config import A3Config, from_flat_dict
...
def _view_confidence_for_replay(o: dict) -> float:
    """Replay 时：frame_quality 为 NONE 表示无有效视图，不因 vc=0 强制 GUARDED，与 runtime 首帧行为对齐。"""
...
def _obs_to_signals(obs_dict: dict) -> A3Signals:
    """OBS_V1.obs 或 record 整条 -> A3Signals。时间来源用 record 的 ts，不由此处处理。"""
    o = obs_dict if isinstance(obs_dict.get("motion"), (int, float)) else (obs_dict.get("obs") or {})
    # path: 0=稳定 1=不稳 -> path_stability = 1 - path；若已是 stability 则直接取
    ...
    if "branch" in o and isinstance(o.get("branch"), (int, float)):
        ...
        if 0 <= b <= 1:
            branch_load = b
    return A3Signals(
        risk_density=float(o.get("complexity", 0.0) or 0.0),
        redline_hit=bool(o.get("redline_hit", False)),
        path_stability=path_stability,
        path_instability=float(o["path"]) if "path" in o and isinstance(o.get("path"), (int, float)) else None,
        ...
        motion_instability=float(o.get("motion", 0.0) or 0.0),
    )


class A3HeadlessAdapter:
    """
    Headless A3：base_config + patch_config 合并后驱动 A3Engine，tick(obs_dict, virtual_ts) 出 decision dict。
    时间仅用 virtual_ts（禁止墙钟）；reset() 清空内部状态保证可重复。
    """
    ...
    def tick(self, obs_dict: dict, virtual_ts: float) -> dict:
        ...
        self._seq = seq + 1
        risk_used = float(getattr(mode, "complexity_score", 0.0))
        threshold = float((getattr(mode, "debug", None) or {}).get("threshold_safe_to_caution", 0.38))
        out: Dict[str, Any] = {
            "seq": seq,
