...
**证据：**
- `advisory_only = True` 已强制声明（2 处）
- 所有消息都是"建议"语义，无"必然发生"表述
- `valid_until` 是"建议有效期"，不是"承诺时间"
...
**代码位置：**
- `b2_v03.py:759-762` - assert 禁止确认性语言
...
**证据：**
- 无"X 秒后一定发生"的表述
- `valid_until` 是"建议有效期窗口"，不是"承诺时间"
- 所有表述都是"风险概率上升"，不是"必然发生"

**代码位置：**
- `b2_v03.py:1082-1092` - valid_until 计算（只是建议窗口）
...
**证据：**
- READ_ONLY 时注释说明"不产生新判断"
- READ_ONLY 时 evidences 可以收集，但不产生新 impact

**代码位置：**
- `b2_v03.py:517-519` - READ_ONLY 处理
