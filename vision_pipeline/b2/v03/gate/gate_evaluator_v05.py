...
class GateEvaluatorV05:
    ...
    def evaluate(
        ...
        *,
        ...
        # Legacy parameters (for compatibility with b2_v03.py)
        pitch_deg: float = 0.0,
        roll_deg: float = 0.0,
        ...
        # Hysteresis "hold band" means: exit_thr <= s < enter_thr
        if exit_thr <= stability_score < enter_thr:
        ...
        # -----------------------------
        # Use evidence_ok or evidence_frames to determine if evidence is sufficient
        if desired_mode == "ACTIVE":
            ...
            elif evidence_frames > 0:
                # Check evidence continuity threshold
                soft_cfg = self.config.get("soft_gates", {}).get("evidence_continuity", {}) or {}
                min_frames = int(soft_cfg.get("consecutive_frames", 6))
    ...
    def _apply_runtime_policy(
        ...
        # Only count a hysteresis hold when we are in the hysteresis band (exit<=s<enter)
        # AND we didn't switch AND runtime policy did not block a switch.
        hysteresis_hold = False
        h = details.get("hysteresis") or {}
        if bool(h.get("in_band")) and (desired_mode == current_mode):
