...
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
log = get_logger("analyze_bottleneck")
"""
...
def main():
    ...
    # 控制台输出
    log.info("=" * 70")
    log.info("Luna Badge 瓶颈分析")
    log.info("=" * 70")
    log.info(f"样本文件: {csv_path.name}")
    log.info(f"场景标记: {base}")
    log.info("")
    log.info(f"平均总延迟: {avg_total:.2f} ms (100%)")
    log.info(f"  - 检测 det: {avg_det:.2f} ms ({pct(avg_det):.1f}%)")
    log.info(f"  - 导航 nav: {avg_nav:.2f} ms ({pct(avg_nav):.1f}%)")
    log.info(f"  - 其他 overhead: {avg_overhead:.2f} ms ({pct(avg_overhead):.1f}%)")
    log.info("")
    ...
    log.info(f"当前主瓶颈模块: {top_name} ({top_val:.2f} ms, {pct(top_val):.1f}%)")
    if second_name and second_val > 0:
        log.info(f"次要瓶颈模块: {second_name} ({second_val:.2f} ms, {pct(second_val):.1f}%)")
    log.info("")
    ...
    if top_name == "det":
        log.info("  ✅ 优先优化检测模型：")
        log.info("     - 使用更小的模型（如 yolo11-tiny）")
        log.info("     - 模型量化（INT8）")
        log.info("     - 降低输入分辨率")
        log.info("     - 模型剪枝")
    elif top_name == "nav":
    ...
    log.info("")
    log.info("=" * 70")
    log.info(f"[INFO] 瓶颈报告已保存：{out_path}")
    log.info("=" * 70")
