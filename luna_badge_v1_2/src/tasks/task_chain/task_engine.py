...
class TaskEngine:
    ...
    def start_main(self, task_id, chain):
        ...
        ctx.start()
        log.info(f"[TaskEngine] Start main task: {task_id}")
    ...
    def insert_task(self, task_id, chain):
        ...
        if self.current_task:
            ...
            log.info(f"[TaskEngine] Pause current task and push to stack: {self.current_task.task_id}")
        
        ctx = TaskContext(task_id, "INSERT", chain)
        self.current_task = ctx
        ctx.start()
        log.info(f"[TaskEngine] Insert task started: {task_id}")
    ...
    def switch_to(self, task_id, chain):
        ...
        if self.main_task:
            ...
            log.info(f"[TaskEngine] Cancel main task: {self.main_task.task_id}")
        
        for ctx in self.stack:
            ctx.cancel()
            log.info(f"[TaskEngine] Cancel stacked task: {ctx.task_id}")
        ...
        self.current_task = ctx
        ctx.start()
        log.info(f"[TaskEngine] Switch to new main task: {task_id}")
    ...
    def force_start(self, task_id, chain):
        ...
        # 若已有强制任务在执行，则忽略
        if self.forced_task:
            log.info(f"[TaskEngine] Force task already running: {self.forced_task.task_id}")
            return
        
        # 暂停当前任务（但不清栈）
        if self.current_task:
            self.current_task.pause()
            log.info(f"[TaskEngine] Pause current task for force task: {self.current_task.task_id}")
        
        ctx = TaskContext(task_id, "FORCE", chain)
        self.forced_task = ctx
        self.current_task = ctx
        ctx.start()
        log.info(f"[TaskEngine] Force task started: {task_id}")
    ...
    def end_force_task(self):
        ...
        log.info(f"[TaskEngine] Force task completed: {self.forced_task.task_id}")
        ...
        if self.stack:
            ...
            log.info(f"[TaskEngine] Resume stacked task: {self.current_task.task_id}")
        elif self.main_task and self.main_task.state == "PAUSED":
            self.current_task = self.main_task
            self.current_task.resume()
            log.info(f"[TaskEngine] Resume main task: {self.main_task.task_id}")
    ...
    def on_task_completed(self, task_ctx: TaskContext):
        ...
        """
        task_ctx.complete()
        log.info(f"[TaskEngine] Task completed: {task_ctx.task_id} (type={task_ctx.task_type})")
        ...
        if task_ctx.task_type == "INSERT":
            ...
            if self.stack:
                ...
                prev.resume()
                log.info(f"[TaskEngine] Resume previous task: {prev.task_id}")
            else:
                self.current_task = self.main_task
                if self.main_task:
            ...
            log.info(f"[TaskEngine] Main task completed, no current task")
    ...
    def try_restore(self, cache: dict):
        ...
        try:
            ...
            log.info(f"[TaskEngine] Restored from cache (timestamp: {cache.get('timestamp')})")
            return True
            
        except Exception as e:
            log.error(f"[TaskEngine] Failed to restore: {e}")
            return False
