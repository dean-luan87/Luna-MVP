# src/vision/models/yolo/yolo_loader.py
...
class UnifiedYOLOLoader:
    ...
    def load_config(self) -> None:
        ...
        if not config_file.exists():
            # 尝试从项目根目录查找（从 src/vision/models/yolo/ 向上4级）
            project_root = Path(__file__).parent.parent.parent.parent.parent
            config_file = project_root / self.config_path
            if not config_file.exists():
                # 尝试从当前工作目录查找
                config_file = Path.cwd() / self.config_path
                if not config_file.exists():
        ...
        if not self.current_model_key:
            raise ValueError("current_nav_model not set in config")

        models_cfg = self.config.get("models", {})
        model_cfg = models_cfg.get(self.current_model_key)
        if not model_cfg:
            raise ValueError(f"model config not found: {self.current_model_key}")
        ...
        if not self.model_path:
            raise ValueError(f"model path missing for: {self.current_model_key}")

        # 支持相对路径（从项目根目录）
        if not os.path.isabs(self.model_path):
            # 从 src/vision/models/yolo/ 向上4级到项目根目录
            project_root = Path(__file__).parent.parent.parent.parent.parent
            ...
            if model_file.exists():
                ...
                # 尝试从当前工作目录查找
                model_file = Path.cwd() / self.model_path
                if model_file.exists():
                    self.model_path = str(model_file)
                else:
        ...
        log.info(
            f"config loaded: key={self.current_model_key}, "
            f"path={self.model_path}, framework={self.framework}, "
    ...
    def load_model(self) -> Any:
        ...
        if self.framework == "pytorch":
            ...
            log.info("✅ model loaded via ultralytics.YOLO (pytorch)")
        else:
    ...
    def infer(self, image) -> Any:
        ...
        if self.framework != "pytorch":
            raise RuntimeError("infer called but framework is not pytorch")
