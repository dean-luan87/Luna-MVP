...
def _get_session_id() -> str:
    """Phase 2.0：进程内唯一 session_id，启动时生成一次。"""
    global _SESSION_ID
...
def _log_a3_v1(obs: Any, decision: Any) -> None:
    """只 dump obs + decision，不做采样、不补值、不推断。Phase 2.0：注入 session_id / version_tag / record_type / 外部感知字段。"""
    payload = {
        ...
        "obs": {
            ...
            "ocr_produced_ts": getattr(obs, "ocr_produced_ts", 0.0),
            "map_hint": getattr(obs, "map_hint", ""),
            "map_produced_ts": getattr(obs, "map_produced_ts", 0.0),
            "speech_event": getattr(obs, "speech_event", ""),
            "speech_produced_ts": getattr(obs, "speech_produced_ts", 0.0),
        },
